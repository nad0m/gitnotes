...
function TableActionMenu({
  ...
  const toggleTableRowIsHeader = useCallback(isHeader => {
    editor.update(() => {
      const tableNode = $getTableNodeFromLexicalNodeOrThrow(tableCellNode);
      const tableRowIndex = $getTableRowIndexFromTableCellNode(tableCellNode);
  ...
  const toggleTableColumnIsHeader = useCallback(isHeader => {
    editor.update(() => {
      const tableNode = $getTableNodeFromLexicalNodeOrThrow(tableCellNode);
      const tableColumnIndex = $getTableColumnIndexFromTableCellNode(tableCellNode);
      const tableRows = tableNode.getChildren();
...
function TableCellActionMenuContainer(): React.MixedElement {
  ...
  return <div className="table-cell-action-button-container" ref={menuButtonRef}>
    {tableCellNode != null && <>
      <button className="table-cell-action-button chevron-down" onClick={e => {
        e.stopPropagation();
        setIsMenuOpen(!isMenuOpen);
      }} ref={menuRootRef}>
        <i className="chevron-down" />
      </button>
      {isMenuOpen && <TableActionMenu contextRef={menuRootRef} setIsMenuOpen={setIsMenuOpen} onClose={() => setIsMenuOpen(false)} tableCellNode={tableCellNode} />}
    </>}
  </div>;
}
