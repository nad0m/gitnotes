...
import { $getTableColumnIndexFromTableCellNode, $getTableNodeFromLexicalNodeOrThrow, $getTableRowIndexFromTableCellNode, $isTableCellNode, $isTableRowNode, getCellFromTarget } from "@lexical/table";
...
function TableCellResizer({
  ...
  const updateRowHeight = useCallback((newHeight: number) => {
    if (!activeCell) {
  ...
  const updateColumnWidth = useCallback((newWidth: number) => {
    if (!activeCell) {
      throw new Error('TableCellResizer: Expected active cell.');
    }
  ...
  return <div ref={resizerRef}>
    {activeCell != null && !isSelectingGrid && <>
      <div className="TableCellResizer__resizer TableCellResizer__ui" style={resizerStyles.right} onMouseDown={toggleResize('right')} onMouseUp={toggleResize('right')} />
      <div className="TableCellResizer__resizer TableCellResizer__ui" style={resizerStyles.bottom} onMouseDown={toggleResize('bottom')} onMouseUp={toggleResize('bottom')} />
    </>}
  </div>;
}

export default function TableCellResizerPlugin(): React.ReactPortal {
  const [editor] = useLexicalComposerContext();
  return useMemo(() => createPortal(<TableCellResizer editor={editor} />, document.body), [editor]);
}