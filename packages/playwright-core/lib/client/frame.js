...
class Frame extends _channelOwner.ChannelOwner {
  ...
  _setupNavigationWaiter(channel, options) {
    ...
    if (this._page.isClosed())
      waiter.rejectImmediately(
        new Error('Navigation failed because page was closed!')
      )
    waiter.rejectOnEvent(
      this._page,
      _events2.Events.Page.Close,
      new Error('Navigation failed because page was closed!')
    )
    waiter.rejectOnEvent(
      this._page,
      _events2.Events.Page.Crash,
      new Error('Navigation failed because page crashed!')
    )
    waiter.rejectOnEvent(
      this._page,
      _events2.Events.Page.FrameDetached,
  ...
  async waitForNavigation(options = {}) {
    return this._page._wrapApiCall(async (channel) => {
      ...
      const toUrl =
        typeof options.url === 'string' ? ` to "${options.url}"` : ''
      ...
      const navigatedEvent = await waiter.waitForEvent(
        ...
        (event) => {
          ...
          return (0, _clientHelper.urlMatches)(
            (_this$_page = this._page) === null || _this$_page === void 0
              ? void 0
              : _this$_page.context()._options.baseURL,
            event.url,
            options.url
          )
        }
      ...
      const response = request
        ? await waiter.waitForPromise(
            request._finalRequest()._internalResponse()
          )
        : null
      waiter.dispose()
      return response
  ...
  async waitForURL(url, options = {}) {
    ...
    if (
      (0, _clientHelper.urlMatches)(
        (_this$_page2 = this._page) === null || _this$_page2 === void 0
          ? void 0
          : _this$_page2.context()._options.baseURL,
        this.url(),
        url
      )
    )
  ...
  async frameElement() {
    return this._wrapApiCall(async (channel) => {
      return _elementHandle.ElementHandle.from(
        (await channel.frameElement()).element
      )
    })
  }
  ...
  async addScriptTag(options = {}) {
    return this._wrapApiCall(async (channel) => {
      ...
      if (copy.path) {
        copy.content = (
          await _fs.default.promises.readFile(copy.path)
        ).toString()
        copy.content += '//# sourceURL=' + copy.path.replace(/\n/g, '')
      }

      return _elementHandle.ElementHandle.from(
        (await channel.addScriptTag({ ...copy })).element
      )
    })
  }
  ...
  async addStyleTag(options = {}) {
    return this._wrapApiCall(async (channel) => {
      ...
      return _elementHandle.ElementHandle.from(
        (await channel.addStyleTag({ ...copy })).element
      )
    })
  }
  ...
  async waitForFunction(pageFunction, arg, options = {}) {
    return this._wrapApiCall(async (channel) => {
      ...
      const result = await channel.waitForFunction({
        ...options,
        pollingInterval:
          options.polling === 'raf' ? undefined : options.polling,
        expression: String(pageFunction),
        isFunction: typeof pageFunction === 'function',
        arg: (0, _jsHandle.serializeArgument)(arg)
      })
