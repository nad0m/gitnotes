...
class Fetch {
  ...
  async newContext(options = {}) {
    return await this._playwright._wrapApiCall(async (channel) => {
      ...
      return FetchRequest.from(
        (
          await channel.newRequest({
            ...options,
...
class FetchRequest extends _channelOwner.ChannelOwner {
  ...
  async fetch(urlOrRequest, options = {}) {
    return this._wrapApiCall(async (channel) => {
      const request =
        urlOrRequest instanceof network.Request ? urlOrRequest : undefined
      ;(0, _utils.assert)(
      ...
      const method =
        options.method ||
        (request === null || request === void 0 ? void 0 : request.method()) // Cannot call allHeaders() here as the request may be paused inside route handler.

      const headersObj =
        options.headers ||
        (request === null || request === void 0 ? void 0 : request.headers())
      const headers = headersObj
        ? (0, _utils.headersObjectToArray)(headersObj)
        : undefined
      let jsonData
      let formData
      let multipartData
      ...
      if (options.data !== undefined) {
        ...
        else if (Buffer.isBuffer(options.data)) postDataBuffer = options.data
        else if (typeof options.data === 'object') jsonData = options.data
