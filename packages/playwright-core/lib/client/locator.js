...
class Locator {
  ...
  async evaluateHandle(pageFunction, arg, options) {
    return this._withElement(
      (h) => h.evaluateHandle(pageFunction, arg),
      options === null || options === void 0 ? void 0 : options.timeout
    )
  }
  ...
  async screenshot(options = {}) {
    return this._withElement(
      (h, timeout) => h.screenshot({ ...options, timeout }),
      options.timeout
    )
  }
  ...
  async selectText(options = {}) {
    return this._withElement(
      (h, timeout) => h.selectText({ ...options, timeout }),
      options.timeout
    )
  }
  ...
  async allInnerTexts() {
    return this._frame.$$eval(this._selector, (ee) =>
      ee.map((e) => e.innerText)
    )
  }

  async allTextContents() {
    return this._frame.$$eval(this._selector, (ee) =>
      ee.map((e) => e.textContent || '')
    )
  }
  ...
  async _expect(expression, options) {
    return this._frame._wrapApiCall(async (channel) => {
      ...
      if (options.expectedValue)
        params.expectedValue = (0, _jsHandle.serializeArgument)(
