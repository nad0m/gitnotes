...
commandWithOpenOptions(
  ...
  [
    ...
    [
      '--target <language>',
      `language to generate, one of javascript, test, python, python-async, csharp`,
      language()
    ]
  ]
)
...
function suggestedBrowsersToInstall() {
  return _registry.registry
    .executables()
    .filter((e) => e.installType !== 'none' && e.type !== 'tool')
    .map((e) => e.name)
    .join(', ')
}
...
function checkBrowsersToInstall(args) {
  ...
  if (faultyArguments.length) {
    console.log(
      `Invalid installation targets: ${faultyArguments
        .map((name) => `'${name}'`)
        .join(', ')}. Expecting one of: ${suggestedBrowsersToInstall()}`
    )
    process.exit(1)
  }
    ...
    try {
      if (!args.length) {
        ...
        if (installDockerImage) {
          const imageName = `mcr.microsoft.com/playwright:v${(0,
          _utils.getPlaywrightVersion)()}-focal`
          const { code } = await (0, _utils.spawnAsync)(
            'docker',
            ['pull', imageName],
            {
              stdio: 'inherit'
...
_commander.program
  ...
  .action(async function (args) {
    try {
      if (!args.length)
        await _registry.registry.installDeps(
          _registry.registry.defaultExecutables()
        )
      else await _registry.registry.installDeps(checkBrowsersToInstall(args))
    } catch (e) {
...
for (const { alias, name, type } of browsers) {
  commandWithOpenOptions(`${alias} [url]`, `open page in ${name}`, [])
    .action(function (url, options) {
      open({ ...options, browser: type }, url, options.target).catch(
        logErrorAndExit
      )
    })
    .addHelpText(
...
commandWithOpenOptions(
  ...
  [
    [
      ...
      'wait for selector before taking a screenshot'
    ],
    [
      '--wait-for-timeout <timeout>',
      'wait for timeout in milliseconds before taking a screenshot'
    ],
    [
      '--full-page',
      'whether to take a full page screenshot (entire scrollable area)'
    ]
  ]
)
...
_commander.program
  .command('experimental-grid-server', {
    hidden: true
  })
  .option('--port <port>', 'grid port; defaults to 3333')
  .option(
    '--agent-factory <factory>',
    'path to grid agent factory or npm package'
  )
...
_commander.program
  .command('experimental-grid-agent', {
    hidden: true
  })
...
_commander.program
  ...
  .option(
    ...
    'browser to use, one of cr, chromium, ff, firefox, wk, webkit',
    'chromium'
  )
  ...
  .action(function (trace, options) {
    ...
    ;(0, _traceViewer.showTraceViewer)(
      trace,
      options.browser,
      false,
      9322
...
if (!process.env.PW_CLI_TARGET_LANG) {
  ...
  if (playwrightTestPackagePath) {
    ...
    {
      const command = _commander.program
        .command('test')
        .allowUnknownOption(true)
...
if (process.argv[2] === 'run-driver') (0, _driver.runDriver)()
else if (process.argv[2] === 'run-server')
  (0, _driver.runServer)(process.argv[3] ? +process.argv[3] : undefined).catch(
    logErrorAndExit
  )
else if (process.argv[2] === 'print-api-json') (0, _driver.printApiJson)()
else if (process.argv[2] === 'launch-server')
  (0, _driver.launchBrowserServer)(process.argv[3], process.argv[4]).catch(
    logErrorAndExit
  )
else _commander.program.parse(process.argv)
...
async function launchContext(options, headless, executablePath) {
  ...
  const contextOptions = // Copy the device descriptor since we have to compare and modify the options.
    options.device ? { ...playwright.devices[options.device] } : {} // In headful mode, use host device scale factor for things to look nice.
  // In headless, keep things the way it works in Playwright by default.
  // Assume high-dpi on MacOS. TODO: this is not perfect.

  if (!headless)
    contextOptions.deviceScaleFactor =
      _os.default.platform() === 'darwin' ? 2 : 1 // Work around the WebKit GTK scrolling issue.
  ...
  if (options.viewportSize) {
    try {
      ...
      console.log(
        'Invalid window size format: use "width, height", for example --window-size=800,600'
      )
      process.exit(0)
    }
  } // Geolocation
  ...
  if (options.geolocation) {
    try {
      const [latitude, longitude] = options.geolocation
        .split(',')
      ...
      console.log(
        'Invalid geolocation format: user lat, long, for example --geolocation="37.819722,-122.478611"'
      )
      process.exit(0)
    }
...
async function open(options, url, language) {
  const { context, launchOptions, contextOptions } = await launchContext(
    options,
...
function commandWithOpenOptions(command, description, options) {
  ...
  return result
    ...
    .option(
      '--lang <language>',
      'specify language / locale, for example "en-GB"'
    )
    .option(
      '--proxy-server <proxy>',
...
async function launchGridServer(factoryPathOrPackageName, port, authToken) {
  if (!factoryPathOrPackageName)
    factoryPathOrPackageName = _path.default.join(
      '..',
      'grid',
      'simpleGridFactory'
    )
