...
class BrowserServerLauncherImpl {
  ...
  async launchServer(options = {}) {
    ...
    const browser = await playwright[this._browserName].launch(
    ...
    if (options.wsPath)
      path = options.wsPath.startsWith('/')
        ? options.wsPath
        : `/${options.wsPath}` // 2. Start the server
  ...
  async _onConnect(playwright, browser, connection, forceDisconnect) {
    ...
    new _dispatcher.Root(connection, async (scope) => {
      ...
      const selectorsDispatcher = new _selectorsDispatcher.SelectorsDispatcher(
        scope,
        selectors
      )
      browserDispatcher = new ConnectedBrowserDispatcher(
        scope,
        browser,
        selectors
      )
    ...
    return () => {
      ...
      ;(_browserDispatcher = browserDispatcher) === null ||
      _browserDispatcher === void 0
        ? void 0
        : _browserDispatcher.cleanupContexts().catch((e) => {})
    }
  }
...
class ConnectedBrowserDispatcher extends _dispatcher.Dispatcher {
  ...
  async cleanupContexts() {
    await Promise.all(
      Array.from(this._contexts).map((context) =>
        context.close((0, _instrumentation.internalCallMetadata)())
      )
    )
  }
}
