...
const dockerFactory = {
  ...
  launch: async (options) => {
    const { vncUrl } = await launchDockerGridAgent(
      options.agentId,
      options.gridURL
    )
...
async function launchDockerGridAgent(agentId, gridURL) {
  ...
  const container = await postJSON('/containers/create', {
    ...
    Cmd: [
      ...
      'start_agent.sh',
      agentId,
      `http://host.docker.internal:${gridPort}`
    ],
    AttachStdout: true,
    AttachStderr: true,
    Image: pwImage.Id,
  ...
  const vncPort =
    info === null || info === void 0
      ? void 0
      : (_info$NetworkSettings = info.NetworkSettings) === null ||
        _info$NetworkSettings === void 0
      ? void 0
      : _info$NetworkSettings.Ports['7900/tcp']
  return {
