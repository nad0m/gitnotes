...
const DOWNLOAD_URLS = {
  ...
  'chromium-with-symbols': {
    ...
    'ubuntu18.04-arm64':
      '%s/builds/chromium/%s/chromium-with-symbols-linux-arm64.zip',
    'ubuntu20.04-arm64':
      '%s/builds/chromium/%s/chromium-with-symbols-linux-arm64.zip',
...
const registryDirectory = (() => {
  ...
  if (envDefined === '0') {
    ...
    if (process.platform === 'linux')
      cacheDirectory =
        process.env.XDG_CACHE_HOME || _path.default.join(os.homedir(), '.cache')
  ...
  constructor(browsersJSON) {
    ...
    this._executables.push({
      ...
      installType: chromium.installByDefault
        ? 'download-by-default'
        : 'download-on-demand',
      validateHostRequirements: (sdkLanguage) =>
