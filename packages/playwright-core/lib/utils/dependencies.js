...
const checkExecutable = (filePath) =>
  _fs.default.promises
    .access(filePath, _fs.default.constants.X_OK)
    .then(() => true)
    .catch((e) => false)
...
function isSupportedWindowsVersion() {
  ...
  const [major, minor] = os
    .release()
    .split('.')
    .map((token) => parseInt(token, 10)) // This is based on: https://stackoverflow.com/questions/42524606/how-to-get-windows-version-using-node-js/44916050#44916050
...
async function installDependenciesWindows(targets) {
  if (targets.has('chromium')) {
    const { code } = await utils.spawnAsync(
      'powershell.exe',
      [
        '-ExecutionPolicy',
        'Bypass',
        '-File',
        _path.default.join(BIN_DIRECTORY, 'install_media_pack.ps1')
      ],
      {
        cwd: BIN_DIRECTORY,
        stdio: 'inherit'
...
async function installDependenciesLinux(targets) {
  ...
  for (const target of targets) {
    if (ubuntuVersion === '18.04') libraries.push(...deps['bionic'][target])
    else if (ubuntuVersion === '20.04') libraries.push(...deps['focal'][target])
...
async function validateDependenciesWindows(windowsExeAndDllDirectories) {
  ...
  const allMissingDeps = await Promise.all(
    lddPaths.map((lddPath) => missingFileDependenciesWindows(lddPath))
  )
  ...
  for (const dep of missingDeps) {
    if (
      dep.startsWith('api-ms-win-crt') ||
      dep === 'vcruntime140.dll' ||
      dep === 'vcruntime140_1.dll' ||
      dep === 'msvcp140.dll'
    )
      isCrtMissing = true
    else if (
...
async function validateDependenciesLinux(
  sdkLanguage,
  linuxLddDirectories,
  dlOpenLibraries
) {
  ...
  const allMissingDeps = await Promise.all(
    lddPaths.map((lddPath) => missingFileDependencies(lddPath, directoryPaths))
  )
  const missingDeps = new Set()
  ...
  libraryToPackageNameMapping = Object.assign(
    {},
    libraryToPackageNameMapping,
  ...
  const maybeSudo =
    process.getuid() !== 0 && os.platform() !== 'win32' ? 'sudo ' : '' // Happy path: known dependencies are missing for browsers.
      ...
      '\n' +
        utils.wrapInASCIIBox(
          [
            ...
            ``,
            `    ${maybeSudo}${(0, _registry.buildPlaywrightCLICommand)(
              sdkLanguage,
              'install-deps'
            )}`,
            ``,
  ...
  if (missingDeps.size) {
    const header = missingPackages.size
...
async function executablesOrSharedLibraries(directoryPath) {
  const allPaths = (await _fs.default.promises.readdir(directoryPath)).map(
    (file) => _path.default.resolve(directoryPath, file)
  )
  const allStats = await Promise.all(
    allPaths.map((aPath) => _fs.default.promises.stat(aPath))
  )
  const filePaths = allPaths.filter((aPath, index) => allStats[index].isFile())
  const executablersOrLibraries = (
    await Promise.all(
...
async function missingFileDependenciesWindows(filePath) {
  const executable = _path.default.join(
    __dirname,
    '..',
    '..',
    'bin',
  ...
  const { stdout, code } = await utils.spawnAsync(executable, [filePath], {
    ...
    env: {
      ...
      LD_LIBRARY_PATH: process.env.LD_LIBRARY_PATH
        ? `${process.env.LD_LIBRARY_PATH}:${dirname}`
        : dirname
    }
  })
  if (code !== 0) return []
  const missingDeps = stdout
    .split('\n')
    .map((line) => line.trim())
    .filter((line) => line.endsWith('not found') && line.includes('=>'))
...
async function missingDLOPENLibraries(libraries) {
  ...
  const { stdout, code, error } = await utils.spawnAsync(
    '/sbin/ldconfig',
    ['-p'],
    {}
  )
  ...
  const isLibraryAvailable = (library) =>
    stdout.toLowerCase().includes(library.toLowerCase())
