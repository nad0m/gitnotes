...
async function getUbuntuVersionAsyncInternal() {
  ...
  let osReleaseText = await _fs.default.promises
    .readFile('/etc/upstream-release/lsb-release', 'utf8')
    .catch((e) => '')
  if (!osReleaseText)
    osReleaseText = await _fs.default.promises
...
function getUbuntuVersionSyncInternal() {
  ...
  try {
    ...
    if (_fs.default.existsSync('/etc/upstream-release/lsb-release'))
      osReleaseText = _fs.default.readFileSync(
        '/etc/upstream-release/lsb-release',
        'utf8'
      )
    else osReleaseText = _fs.default.readFileSync('/etc/os-release', 'utf8')
    if (!osReleaseText) return ''
    return parseUbuntuVersion(osReleaseText)
  } catch (e) {
    return ''
...
function parseUbuntuVersion(osReleaseText) {
  ...
  if (
    fields.get('distrib_id') &&
    fields.get('distrib_id').toLowerCase() === 'ubuntu'
  )
