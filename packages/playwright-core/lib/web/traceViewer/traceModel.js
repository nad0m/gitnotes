...
class TraceModel {
  ...
  appendEvent(line) {
    ...
    switch (event.type) {
      ...
      case 'action': {
        const include =
          !isTracing(event.metadata) &&
          (!event.metadata.internal || event.metadata.apiName)

        if (include) {
          if (!event.metadata.apiName)
            event.metadata.apiName =
              event.metadata.type + '.' + event.metadata.method
      ...
      case 'event': {
        ...
        if (metadata.pageId) {
          if (metadata.method === '__create__')
            this.contextEntry.objects[metadata.params.guid] =
