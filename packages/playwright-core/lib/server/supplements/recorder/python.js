...
class PythonLanguageGenerator {
  ...
  generateAction(actionInContext) {
    ...
    if (action.name === 'openPage') {
      ...
      if (
        ...
        formatter.add(
          `${this._awaitPrefix}${pageAlias}.goto(${quote(action.url)})`
        )
    ...
    if (signals.assertNavigation)
      formatter.add(
        `  # assert ${pageAlias}.url == ${quote(signals.assertNavigation.url)}`
      )
    return formatter.format()
  }
  ...
  _generateActionCall(action) {
    switch (action.name) {
      ...
      case 'setInputFiles':
        return `set_input_files(${quote(action.selector)}, ${formatValue(
      ...
      case 'select':
        return `select_option(${quote(action.selector)}, ${formatValue(
...
function formatOptions(value, hasArguments) {
  ...
  return (
    (hasArguments ? ', ' : '') +
    keys
      .map((key) => `${toSnakeCase(key)}=${formatValue(value[key])}`)
      .join(', ')
  )
}
...
function formatContextOptions(options, deviceName) {
  ...
  return (
    ...
    formatOptions((0, _language.sanitizeDeviceOptions)(device, options), true)
  )
}
