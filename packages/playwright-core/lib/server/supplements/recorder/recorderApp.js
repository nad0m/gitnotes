...
class RecorderApp extends _events.EventEmitter {
  ...
  async _init() {
          ...
          headers: [
            {
              name: 'Content-Type',
              value:
                mime.getType(_path.default.extname(file)) ||
    ...
    await this._page.exposeBinding('dispatch', false, (_, data) =>
    ...
    const context = await recorderPlaywright.chromium.launchPersistentContext(
      ...
      '',
      {
        ...
        headless:
          ...
          ((0, _utils.isUnderTest)() && !process.env.HEADFUL),
  ...
  async setSources(sources) {
    await this._page
      ...
      .catch(() => {}) // Testing harness for runCLI mode.
