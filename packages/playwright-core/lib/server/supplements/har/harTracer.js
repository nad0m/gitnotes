  ...
  start() {
    ...
    this._eventListeners = [
      _eventsHelper.eventsHelper.addEventListener(
        ...
        (page) => this._ensurePageEntry(page)
      ),
      _eventsHelper.eventsHelper.addEventListener(
        this._context,
        _browserContext.BrowserContext.Events.Request,
        (request) => this._onRequest(request)
      ),
      _eventsHelper.eventsHelper.addEventListener(
        this._context,
    ...
    const compressionCalculationBarrier = {
      ...
      _check: function () {
        if (this._encodedBodySize !== -1 && this._decodedBodySize !== -1) {
          harEntry.response.content.compression = Math.max(
            0,
            this._decodedBodySize - this._encodedBodySize
          )
    ...
    const promise = response
        ...
        if (buffer && buffer.length > 0) {
          if (this._options.content === 'embedded') {
            ...
            content._sha1 =
              ...
              '.' +
              (mime.getExtension(content.mimeType) || 'dat')
            if (this._started)
      ...
      .then(() => {
        ...
        if (
          ...
          this._options.content === 'sha1'
        ) {
          harEntry.request.postData._sha1 =
            (0, _utils.calculateSha1)(postData) +
            '.' +
            (mime.getExtension(harEntry.request.postData.mimeType) || 'dat')
          if (this._started)
            this._delegate.onContentBlob(
              harEntry.request.postData._sha1,
              postData
            )
        }
  ...
  _onResponse(response) {
    ...
    const dns =
      timing.domainLookupEnd !== -1
        ? _helper.helper.millisToRoundishMillis(
    ...
    const wait =
      timing.responseStart !== -1
        ...
        : -1
    const receive =
      response.request()._responseEndTiming !== -1
        ? _helper.helper.millisToRoundishMillis(
            response.request()._responseEndTiming - timing.responseStart
          )
        : -1
    harEntry.timings = {
      dns,
    ...
    harEntry.time = [dns, connect, ssl, wait, receive].reduce(
    ...
    this._addBarrier(
      page,
      request.rawRequestHeaders().then((headers) => {
        for (const header of headers.filter(
          (header) => header.name.toLowerCase() === 'cookie'
        ))
          harEntry.request.cookies.push(
            ...header.value.split(';').map(parseCookie)
          )
    ...
    this._addBarrier(
      page,
      response.rawResponseHeaders().then((headers) => {
        for (const header of headers.filter(
  ...
  stop() {
    ...
    for (const pageEntry of log.pages) {
      if (pageEntry.pageTimings.onContentLoad >= 0)
        pageEntry.pageTimings.onContentLoad -=
          pageEntry.startedDateTime.valueOf()
...
function postDataForHar(request, content) {
  ...
  if (!postData) return
  const contentType =
    request.headerValue('content-type') || 'application/octet-stream'
  const result = {
...
function parseCookie(c) {
  ...
  for (const pair of c.split(/; */)) {
    ...
    const name =
      indexOfEquals !== -1 ? pair.substr(0, indexOfEquals).trim() : pair.trim()
    const value =
      indexOfEquals !== -1
        ? pair.substr(indexOfEquals + 1, pair.length).trim()
        : ''
