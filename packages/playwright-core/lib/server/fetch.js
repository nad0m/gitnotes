...
class FetchRequest extends _instrumentation.SdkObject {
  ...
  async fetch(params) {
    try {
      ...
      if (proxy) {
        ...
        if (
          (_proxyOpts$protocol = proxyOpts.protocol) !== null &&
          _proxyOpts$protocol !== void 0 &&
          _proxyOpts$protocol.startsWith('socks')
        ) {
  ...
  async _sendRequest(url, options, postData) {
    ...
    return new Promise((fulfill, reject) => {
      ...
      const request = requestConstructor(url, options, async (response) => {
        ...
        if (redirectStatus.includes(response.statusCode)) {
          ...
          if (options.rejectUnauthorized === false)
            redirectOptions.rejectUnauthorized = false // HTTP-redirect fetch step 4: If locationURL is null, then return response.
      ...
      request.on('close', () =>
        this.off(FetchRequest.Events.Dispose, disposeListener)
      )
