 ...
 */
const ARTIFACTS_FOLDER = _path.default.join(
  os.tmpdir(),
...
class BrowserType extends _instrumentation.SdkObject {
  ...
  executablePath() {
    return (
      _registry.registry
        .findExecutable(this._name)
        .executablePath(this._playwrightOptions.sdkLanguage) || ''
    )
  }
  ...
  async launch(metadata, options, protocolLogger) {
    ...
    const browser = await controller.run((progress) => {
      const seleniumHubUrl =
        options.__testHookSeleniumRemoteURL || process.env.SELENIUM_REMOTE_URL
      if (seleniumHubUrl)
      ...
      return this._innerLaunchWithRetries(
        ...
        options,
        undefined,
        _helper.helper.debugProtocolLogger(protocolLogger)
      ).catch((e) => {
  ...
  async launchPersistentContext(metadata, userDataDir, options) {
    ...
    const browser = await controller.run((progress) => {
      return this._innerLaunchWithRetries(
        ...
        options,
        persistent,
        _helper.helper.debugProtocolLogger(),
        userDataDir
      ).catch((e) => {
        throw this._rewriteStartupError(e)
      })
  ...
  async _innerLaunch(
    ...
    options.proxy = options.proxy
      ? (0, _browserContext.normalizeProxySettings)(options.proxy)
      : undefined
    const browserLogsCollector = new _debugLogger.RecentLogsCollector()
    const { browserProcess, artifactsDir, transport } =
    ...
    if (persistent)
      (0, _browserContext.validateBrowserContextOptions)(
        persistent,
        browserOptions
      )
  ...
  async _launchProcess(
    ...
    const env = options.env
      ? (0, _processLauncher.envArrayToObject)(options.env)
      : process.env
    ...
    if (userDataDir) {
      ...
      userDataDir = await _fs.default.promises.mkdtemp(
        _path.default.join(os.tmpdir(), `playwright_${this._name}dev_profile-`)
      )
    ...
    if (ignoreAllDefaultArgs) browserArguments.push(...args)
      ...
      browserArguments.push(
        ...this._defaultArgs(options, isPersistent, userDataDir)
      )
    let executable
    ...
    if (executablePath) {
      ...
      const registryExecutable = _registry.registry.findExecutable(
        options.channel || this._name
      )
      ...
      executable = registryExecutable.executablePathOrDie(
        this._playwrightOptions.sdkLanguage
      )
    ...
    const shouldWaitForWSListening =
      options.useWebSocket ||
      ((_options$args = options.args) === null || _options$args === void 0
        ? void 0
        : _options$args.some((a) => a.startsWith('--remote-debugging-port')))
    const waitForWSEndpoint = shouldWaitForWSListening
      ? new Promise((f) => (wsEndpointCallback = f))
      : undefined // Note: it is important to define these variables before launchProcess, so that we don't get
    // "Cannot access 'browserServer' before initialization" if something went wrong.

    let transport = undefined
    let browserProcess = undefined
    const { launchedProcess, gracefullyClose, kill } = await (0,
    _processLauncher.launchProcess)({
      command: executable,
      args: browserArguments,
      env: this._amendEnvironment(
        env,
        userDataDir,
        executable,
        browserArguments
      ...
      attemptToGracefullyClose: async () => {
        if (options.__testHookGracefullyClose)
          await options.__testHookGracefullyClose() // We try to gracefully close to prevent crash reporting and core dumps.
    ...
    browserProcess = {
      ...
      close: () =>
        closeOrKill(
          options.__testHookBrowserCloseTimeout ||
            _timeoutSettings.DEFAULT_TIMEOUT
        ),
      kill
    }
    ...
    if (options.useWebSocket) {
      transport = await _transport.WebSocketTransport.connect(
        progress,
        wsEndpoint
      )
    } else {
