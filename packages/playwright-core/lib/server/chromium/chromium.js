...
class Chromium extends _browserType.BrowserType {
  ...
  async connectOverCDP(metadata, endpointURL, options, timeout) {
    ...
    return controller.run(
      async (progress) => {
        return await this._connectOverCDPInternal(
          progress,
          endpointURL,
          options
        )
  ...
  async _connectOverCDPInternal(progress, endpointURL, options, onClose) {
    ...
    const chromeTransport = await _transport.WebSocketTransport.connect(
      progress,
      wsEndpoint,
      headersMap
    )
  ...
  _createDevTools() {
    ...
    return directory
      ? new _crDevTools.CRDevTools(
          _path.default.join(directory, 'devtools-preferences.json')
        )
      : undefined
  }
  ...
  _rewriteStartupError(error) {
    ...
    if (
      ...
      return error
    return (0, _stackTrace.rewriteErrorMessage)(
      error,
      [
  ...
  async _launchWithSeleniumHub(progress, hubUrl, options) {
    ...
    const response = await (0, _utils.fetchData)(
      ...
      async (response) => {
        ...
        return new Error(
          `Error connecting to Selenium at ${hubUrl}: ${message}`
        )
      }
    )
    ...
    const disconnectFromSelenium = async () => {
      ...
      await (0, _utils.fetchData)({
        ...
        progress.log(`<error disconnecting from selenium>: ${error}`)
      )
    ...
    try {
      ...
      const chromeOptions =
        maybeChromeOptions && typeof maybeChromeOptions === 'object'
          ? maybeChromeOptions
          : undefined
      const debuggerAddress =
        chromeOptions && typeof chromeOptions.debuggerAddress === 'string'
          ? chromeOptions.debuggerAddress
          : undefined
      const chromeOptionsURL =
        typeof maybeChromeOptions === 'string' ? maybeChromeOptions : undefined
      let endpointURL =
        capabilities['se:cdp'] || debuggerAddress || chromeOptionsURL
      if (
        !['ws://', 'wss://', 'http://', 'https://'].some((protocol) =>
          endpointURL.startsWith(protocol)
        )
      )
        endpointURL = 'http://' + endpointURL
      return this._connectOverCDPInternal(
        progress,
        endpointURL,
        {
          slowMo: options.slowMo
  ...
  _innerDefaultArgs(options) {
    ...
    if (proxy) {
      ...
      if (proxyBypassRules.length > 0)
        chromeArguments.push(
          `--proxy-bypass-list=${proxyBypassRules.join(';')}`
        )
    }
...
async function urlToWSEndpoint(endpointURL) {
  ...
  const httpURL = endpointURL.endsWith('/')
    ? `${endpointURL}json/version/`
    : `${endpointURL}/json/version/`
  const request = endpointURL.startsWith('https')
    ? _https.default
    : _http.default
