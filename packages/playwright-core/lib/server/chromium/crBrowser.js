...
class CRBrowser extends _browser.Browser {
  static async connect(transport, options, devtools) {
    const connection = new _crConnection.CRConnection(
      transport,
    ...
    browser._version = version.product.substring(
      version.product.indexOf('/') + 1
    )
    ...
    browser._defaultContext = new CRBrowserContext(
      browser,
      undefined,
      options.persistent
    )
  ...
  constructor(connection, options) {
    ...
    this._connection.on(_crConnection.ConnectionEvents.Disconnected, () =>
      this._didClose()
    )
    ...
    this._session.on(
      ...
      this._onDownloadWillBegin.bind(this)
    )

    this._session.on(
      'Browser.downloadProgress',
      this._onDownloadProgress.bind(this)
    )
  }

  async newContext(options) {
    ;(0, _browserContext.validateBrowserContextOptions)(options, this.options)
    const { browserContextId } = await this._session.send(
  ...
  async _waitForAllPagesToBeInitialized() {
    await Promise.all(
      [...this._crPages.values()].map((page) => page.pageOrError())
    )
  }
  ...
  _onAttachedToTarget({ targetInfo, sessionId, waitingForDebugger }) {
    ...
    ;(0, _utils.assert)(
      !this._crPages.has(targetInfo.targetId),
      'Duplicate target ' + targetInfo.targetId
    )
    ;(0, _utils.assert)(
      !this._backgroundPages.has(targetInfo.targetId),
      'Duplicate target ' + targetInfo.targetId
    )
    ;(0, _utils.assert)(
      !this._serviceWorkers.has(targetInfo.targetId),
      'Duplicate target ' + targetInfo.targetId
    )

    if (targetInfo.type === 'background_page') {
      const backgroundPage = new _crPage.CRPage(
        session,
        targetInfo.targetId,
        context,
        null,
    ...
    if (targetInfo.type === 'page') {
      const opener = targetInfo.openerId
        ? this._crPages.get(targetInfo.openerId) || null
        : null
      const crPage = new _crPage.CRPage(
        session,
        targetInfo.targetId,
        context,
        opener,
    ...
    if (targetInfo.type === 'service_worker') {
      const serviceWorker = new CRServiceWorker(
        context,
        session,
        targetInfo.url
      )
  ...
  _onDownloadWillBegin(payload) {
    ...
    ;(0, _utils.assert)(
      page,
      'Download started in unknown page: ' + JSON.stringify(payload)
    )
  ...
  async startTracing(page, options = {}) {
    ;(0, _utils.assert)(
      !this._tracingRecording,
      'Cannot start recording trace while already recording trace.'
    )
    this._tracingClient = page
      ? page._delegate._mainFrameSession._client
      : this._session
  ...
  async stopTracing() {
    ...
    const [event] = await Promise.all([
      new Promise((f) =>
        this._tracingClient.once('Tracing.tracingComplete', f)
      ),
      this._tracingClient.send('Tracing.end')
    ])
    const result = await (0, _crProtocolHelper.readProtocolStream)(
      this._tracingClient,
      event.stream,
      this._tracingPath
    )
...
class CRBrowserContext extends _browserContext.BrowserContext {
  ...
  async _initialize() {
    ;(0, _utils.assert)(
      !Array.from(this._browser._crPages.values()).some(
        (page) => page._browserContext === this
      )
    )
    const promises = [super._initialize()]

    if (
      this._browser.options.name !== 'electron' &&
      this._browser.options.name !== 'clank'
    ) {
      promises.push(
  ...
  async newPageDelegate() {
    ...
    const oldKeys = this._browser.isClank()
      ? new Set(this._browser._crPages.keys())
      : undefined
    let { targetId } = await this._browser._session.send(
      'Target.createTarget',
      {
        url: 'about:blank',
        browserContextId: this._browserContextId
  ...
  _onClosePersistent() {
    ...
    for (const [
      targetId,
      backgroundPage
    ] of this._browser._backgroundPages.entries()) {
      if (
        backgroundPage._browserContext === this &&
        backgroundPage._initializedPage
      ) {
        backgroundPage.didClose()
  ...
  serviceWorkers() {
    return Array.from(this._browser._serviceWorkers.values()).filter(
      (serviceWorker) => serviceWorker._browserContext === this
    )
  }
