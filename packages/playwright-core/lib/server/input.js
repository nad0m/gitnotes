...
class Keyboard {
  ...
  _keyDescriptionForString(keyString) {
    ...
    description =
      shift && description.shifted ? description.shifted : description // if any modifiers besides shift are pressed, no text should be sent

    if (
      this._pressedModifiers.size > 1 ||
      (!this._pressedModifiers.has('Shift') &&
        this._pressedModifiers.size === 1)
    )
  ...
  async imeSetComposition(text, selectionStart, selectionEnd, options) {
    ...
    await this._raw.imeSetComposition(
      text,
      selectionStart,
      selectionEnd,
      replacementStart,
...
function buildLayoutClosure(layout) {
  ...
  for (const code in layout) {
    ...
    result.set(code, { ...description, shifted: shiftedDescription }) // Map from aliases: Shift -> non-shiftable definition
