...
class ElectronApplication extends _instrumentation.SdkObject {
  ...
  async close() {
    const progressController = new _progress.ProgressController(
      (0, _instrumentation.internalCallMetadata)(),
      this
    )
    const closed = progressController.run(
      (progress) =>
        _helper.helper.waitForEvent(
          progress,
          this,
          ElectronApplication.Events.Close
        ).promise,
      this._timeoutSettings.timeout({})
    )
...
class Electron extends _instrumentation.SdkObject {
  ...
  async launch(options) {
    ...
    return controller.run(async (progress) => {
      ...
      const nodeTransport = await _transport.WebSocketTransport.connect(
        progress,
        nodeMatch[1]
      )
      const nodeConnection = new _crConnection.CRConnection(
        nodeTransport,
