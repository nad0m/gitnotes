...
class Screenshotter {
  ...
  async _fullPageSize(progress) {
    const fullPageSize = await this._page
      .mainFrame()
      .waitForFunctionValueInUtility(progress, () => {
  ...
  async screenshotPage(progress, options) {
    ...
    return this._queue.postTask(async () => {
      const { viewportSize, originalViewportSize } =
      ...
      if (options.fullPage) {
        ...
        const fitsViewport =
          fullPageSize.width <= viewportSize.width &&
          fullPageSize.height <= viewportSize.height

        if (
          !this._page._delegate.canScreenshotOutsideViewport() &&
          !fitsViewport
        ) {
          ...
          progress.cleanupWhenAborted(() =>
            this._restoreViewport(originalViewportSize)
          )
        }
  ...
  async screenshotElement(progress, handle, options = {}) {
    ...
    return this._queue.postTask(async () => {
      ...
      const buffer = await this._screenshot(
        progress,
        format,
        _helper.helper.enclosingIntRect(documentRect),
        undefined,
        options
      )
      progress.throwIfAborted() // Avoid restoring after failure - should be done by cleanup.
  ...
  async _screenshot(progress, format, documentRect, viewportRect, options) {
    ...
    if (shouldSetDefaultBackground) {
      ...
      progress.cleanupWhenAborted(() =>
        this._page._delegate.setBackgroundColor()
      )
    }

    progress.throwIfAborted() // Avoid extra work.

    const buffer = await this._page._delegate.takeScreenshot(
      progress,
      format,
      documentRect,
      viewportRect,
  ...
  async _restoreViewport(originalViewportSize) {
    ...
    else await this._page._delegate.resetViewport()
  }
}
