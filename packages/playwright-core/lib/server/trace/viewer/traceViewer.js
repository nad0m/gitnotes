...
async function showTraceViewer(traceUrl, browserName, headless = false, port) {
  ...
  const traceViewerBrowser = (0, _utils.isUnderTest)()
    ? 'chromium'
    : browserName
  const args =
    traceViewerBrowser === 'chromium'
      ? ['--app=data:text/html,', '--window-size=1280,800']
      : []
  if ((0, _utils.isUnderTest)()) args.push(`--remote-debugging-port=0`)
  const context = await traceViewerPlaywright[
    traceViewerBrowser
  ].launchPersistentContext((0, _instrumentation.internalCallMetadata)(), '', {
    // TODO: store language in the trace.
    channel: (0, _registry.findChromiumChannel)(
      traceViewerPlaywright.options.sdkLanguage
    ),
    args,
  ...
  if ((0, _utils.isUnderTest)())
    page.on('close', () =>
      context
        .close((0, _instrumentation.internalCallMetadata)())
        .catch(() => {})
    )
  ...
  await page
    ...
    .goto(
      (0, _instrumentation.internalCallMetadata)(),
      urlPrefix + `/trace/index.html${traceUrl ? '?trace=' + traceUrl : ''}`
    )
  return context
}
